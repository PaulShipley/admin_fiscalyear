...
/**********************************************************************
// This is a modified version of the original fiscalyears.php
// The purpose of this modification is to allow the start date of
// the company fiscal year to be changed, and to simplify the UI
***********************************************************************/
...
function handle_submit()
{
	...
	if ($selected_id != -1)
	{
		...
		if ($ok)
		{
   			update_fiscalyear_replacement($selected_id, $_POST['from_date'], $_POST['to_date'], $_POST['closed']);
...
function update_fiscalyear_replacement($id, $from_date, $to_date, $closed)
{
	$from = date2sql($from_date);
	$to = date2sql($to_date);

	$sql = "UPDATE ".TB_PREF."fiscal_year SET begin=".db_escape($from).",end=".db_escape($to).",closed=".db_escape($closed)."
		WHERE id=".db_escape($id);

	db_query($sql, "could not update fiscal year");
}

function yesno_value($value)
{
	return ($value == 0) ? _("No") : _("Yes");
}
...
function display_fiscalyears()
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		label_cell($from);
		label_cell($to);
		label_cell(yesno_value($myrow["closed"]));
